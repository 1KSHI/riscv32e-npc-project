...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhu  , R, R(rd) = ((uint64_t)src1 * (uint64_t)src2) >> 32);
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhsu , R, R(rd) = SEXT(src1,64) * (uint64_t)src2 >> 32);
  //I-type
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, {R(rd) = s->pc + 4; s->dnpc = (src1 + imm); IFDEF(CONFIG_FTRACE,{identity_cal_ret("jalr", s);}) });
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(Mr(src1 + imm, 1),8));//SEXT(Mr(src1 + imm, 1),8)
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, 2),16));//SEXT(Mr(src1 + imm, 2),16)
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = SEXT(Mr(src1 + imm, 4),32));//SEXT(Mr(src1 + imm, 4),32)
  ...
  INSTPAT("? ?????????? ? ???????? ????? 11011 11", jal    , J, {R(rd) = s->pc + 4; s->dnpc = s->pc + imm;IFDEF(CONFIG_FTRACE, {identity_cal_ret("jal", s);}) });
...
int isa_exec_once(Decode *s) {
  s->isa.inst = inst_fetch(&s->snpc, 4);//从内存读取指令地址，并且传入指令长度4,更新snpc（下一个静态地址）
